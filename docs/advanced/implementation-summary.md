...
- **Release Workflow**: `.github/workflows/release.yml` validates the container image build on pull requests and, on push/manual runs, versions/builds/pushes it via `scripts/docs-build-image.ps1` (`:latest` + `:YYYY.MM.DD`), then deploys Pages.
