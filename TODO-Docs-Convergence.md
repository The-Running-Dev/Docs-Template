...
- [ ] Fold the gate job from `docs-quality.yml` into
...
- [ ] Add `push: branches: [main]` to `scripts/template/docs-deploy.yml`.
- [ ] Declare the triggers without `paths:` filters, per the decision above, so
      a required check always reports.
- [ ] Bump both the payload and this repository's own workflows to the target
      action versions in the table above, then confirm with a real CI run that
      Node 24 actions still work inside the Alpine container.
- [ ] Delete `scripts/template/docs.yml` (pure caller indirection) and
      `scripts/template/docs-quality.yml` (folded in).
- [ ] Delete `scripts/setup-docs-workflow.ps1`; fold its two-file install back
      into `setup-docs.ps1`, removing the split-reporting note at
      `scripts/setup-docs.ps1:487-492`.
- [ ] Rework `-SkipGate` so it excises a *job* from `docs-ci.yml` rather than
      skipping a whole file, using the same start/end marker technique the
      `GeneratedFiles` block already uses.
- [ ] Keep job `name:` values byte-identical — `Documentation links and
      terminology`, `Verify Documentation Build`, `Build and Deploy
      Documentation` — so any required-status-check context keeps matching.
- [ ] Copy the workflow from the payload. Do **not** embed it as a here-string
      the way `planning/Install-DocsSystem.ps1:461-570` does; that reintroduces
      the drift this effort removes.
- [ ] Update `AGENTS.md` for the two-workflow shape, replacing any reference to
      the retired `docs.yml`, `docs-quality.yml`, and `setup-docs-workflow.ps1`.

Acceptance criteria:

- A scratch install produces exactly two workflow files.
- Both pass `actionlint`.
- The gate job carries no `pages` or `id-token` permission.
- `-SkipGate` and `-WhatIf` still behave as documented.
